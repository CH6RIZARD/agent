...
**Why you might see that phrase even when chat works:** (1) The agent injects `last_autonomous_message` into the chat system prompt; the code now avoids storing and injecting the fallback, so the LLM is not prompted that way. (2) The chat system prompt used to always include a "degraded mode" instruction: "If you are told the model is unreachable, say in plain language that the model is unreachable." So when the **first** chat call failed and the short fallback went into conversation history, the **next** turn the (working) chat LLM saw that and elaborated with long "I'm unable to connect to a reasoning model..." paragraphs. So it looked like the agent "could talk but not reason"—in fact the long replies were from the working LLM describing the earlier failure. The prompt now says: "If you are replying to this message, the model is in use—do not say the model or your reasoning is unreachable," so when chat succeeds the model no longer claims it is unreachable.

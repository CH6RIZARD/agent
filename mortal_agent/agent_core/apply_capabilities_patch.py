"""Apply explicit capabilities + judgment framing to identity.py (fix read-only confusion)."""
import os

path = os.path.join(os.path.dirname(__file__), "identity.py")
with open(path, "r", encoding="utf-8") as f:
    content = f.read()
content = content.replace("\r\n", "\n").replace("\r", "\n")

old_block = '''CAPABILITIES_STATEMENT = (
...
new_block = '''CAPABILITIES_STATEMENT = (
...
if old_block not in content:
    print("Block not found; identity.py may already be patched or format changed.")
    raise SystemExit(1)

content = content.replace(old_block, new_block, 1)
with open(path, "w", encoding="utf-8") as f:
    f.write(content)
print("Patched identity.py: explicit capabilities + judgment framing applied.")
